#!/usr/bin/env python
import sys
sys.path.append('/home/artemis/software/presto/lib/python/')
import bisect, os, getopt, infodata, glob
    ...
    if (opts.makeplot):
        ...
        if pgplot_device:
            ...
            if (opts.T_start > 0.0 or opts.T_end < obstime):
                ppgplot.pgopen(short_filenmbase+'_%.0f-%.0fs_ini_singlepulse.ps/VPS'%
                               (opts.T_start, opts.T_end))
            else:
                ppgplot.pgopen(short_filenmbase+'_ini_singlepulse.ps/VPS')
        ...
        borderDM = (float(max(DMs))- float(min(DMs)))/32.
	#print(borderDM)
	#print(DMs)
	#print(max(DMs))
	#print(min(DMs))
	#print(min(DMs)-0.001, max(DMs)+0.001, 0.0, 1.1*max(num_v_DM+[1]))

        #print(borderDM)
        ppgplot.pgswin(min(DMs)-0.001, max(DMs)+0.001, 0.0, 1.1*max(num_v_DM+[1]))
        ppgplot.pgsch(0.8)
        ...
        ppgplot.pgswin(min(DMs)- 0.001, max(DMs)+0.001, opts.threshold, maxsnr)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
        ppgplot.pgmtxt('B', 2.5, 0.5, 0.5, "DM (pc cm\u-3\d)")
        ...
        if (maxsnr == 100): cand_SNRs[(cand_SNRs > 100)] = 100.
        ppgplot.pgpt(cand_DMs, cand_SNRs, 20)
        
        #print("%.4f" % cand_DMs[99])

        # plot the DM vs Time plot
        ppgplot.pgsvp(0.06, 0.97, 0.08, 0.52)
        ppgplot.pgswin(opts.T_start, opts.T_end, min(DMs)-0.001, max(DMs)+0.001)
        ppgplot.pgsch(0.8)
        ppgplot.pgbox("BCNST", 0, 0, "BCNST", 0, 0)
        ...
        for ii in [26, 25, 24, 23, 22, 21, 20]:
            ...
            #print(cand_DMs[inds])
            #if(len(cand_DMs[inds]) > 1): print("%u %.4f" % (ii, cand_DMs[inds][0]))
            #if(len(cand_DMs[inds]) > 1): print("%u %.4f" % (ii, cand_DMs[inds][4]))
...
if __name__ == '__main__':
    ...
    if (0):
        ...
        if opts.makeplot :
            ...
            res = []
            ind = 0
            while ( ind * part ) < len( args ) :
                work_args = args[ ind * part : ( ind + 1 ) * part ]
                ind += 1
