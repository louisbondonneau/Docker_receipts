...
#	Modify the SP threshold used to take the lowest between the theoritical and the user input.
...
def main( args ):
    ...
    if not dosearch:
        ...
        #	th_thresh = ( Num.sqrt( Num.log( opts.nSamp * opts.nDM ) ) - Num.sqrt( Num.pi / 4 ) ) / Num.sqrt( 1 - Num.pi / 4 )
        #	if opts.threshold > th_thresh :
        #		opts.threshold = th_tresh
    ...
    if (opts.makeplot):
        ...
        print("Best candidate DM = %.6f pc cm-3 SNR = %.2f time = %.2f sec" % (cand_DMs[Num.argmax(cand_SNRs)], Num.max(cand_SNRs), cand_ts[Num.argmax(cand_SNRs)]))
