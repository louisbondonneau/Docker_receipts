...
newDirName = os.path.dirname(args.newFileName)
if not (newDirName == ''):
...
old_data = np.memmap(newDirName + 'tmp_memmapped_' + os.path.basename(args.fileName), dtype=np.float32,
...
newCol = fi.Column(name=oldCol_offset.name, format=newFormat_offset, unit=oldCol_offset.unit,
                   dim='(1,' + str(nchan) + ',' + str(npol) + ')', array=OFFSET)    # Creation of the new field
colList.append(newCol)
newCol = fi.Column(name=oldCol_scale.name, format=newFormat_scale, unit=oldCol_scale.unit,
                   dim='(1,' + str(nchan) + ',' + str(npol) + ')', array=SCAL)    # Creation of the new field
colList.append(newCol)
...
newCol = fi.Column(name=oldCol_data.name, format=newFormat_data, unit=oldCol_data.unit, dim='(1,' + str(nchan) + ',' +
                   str(npol) + ',' + str(line_lenght) + ')', array=old_data.astype('uint8'))    # Creation of the new field
colList.append(newCol)                        # Adding to the new field list
...
os.remove(newDirName + 'tmp_memmapped_' + os.path.basename(args.fileName))
